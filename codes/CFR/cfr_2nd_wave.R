...
data_state_cases = data_state_cases[which(data_state_cases["Date"] == start_date):which(data_state_cases["Date"] == target_date),]
data_state_deaths = data_state[data_state["Status"] == "Deceased", ]
data_state_deaths = data_state_deaths[which(data_state_deaths["Date"] == start_date):which(data_state_deaths["Date"] == target_date),]
...
date_initial = which(data$Date_YMD == start_date_india_format) - 1
...
observed_cum_cases = c(observed_cum_cases, "India" = tail(data_country_confirmed,1) - head(data_country_confirmed,1))
observed_cum_deaths = colSums(data_state_deaths[, state_codes[-length(state_codes)]])
observed_cum_deaths = c(observed_cum_deaths, "India" = tail(data_country_deaths,1) - head(data_country_deaths,1))
...
saveRDS(df_cfr,paste0("CFR_CI-", start_date,"_to_", target_date, ".rds"))
